...
bool SetEnvironmentTempPath() {
    ...
    REX::INFO("Mantella temp files in {}", WideStringToString(newTempPath.c_str()));
...
bool LaunchMantellaExe() {
    ...
    REX::INFO("EXE path: {}", exePathStr2);
    RE::ConsoleLog::GetSingleton()->PrintLine("Attempting to launch: %s", exePathStr2.c_str());
...
// F4SEPlugin_Version is generated by the commonlibf4.plugin xmake rule (see xmake.lua).
// F4SE::Init opens Documents/My Games/Fallout4/F4SE/MantellaLauncher.log.
F4SE_PLUGIN_LOAD(const F4SE::LoadInterface* a_f4se) {
    F4SE::Init(a_f4se, { .logName = "MantellaLauncher" });

    REX::INFO("MantellaLauncher {} loaded", F4SE::GetPluginVersion().string());
